...
def get_config(catalog: str = "zivile", schema: str = "sap_demo"):
    """
    Get or setup Unity Catalog configuration
    
    Args:
        catalog: Catalog name (default: "zivile")
        schema: Schema name (default: "sap_demo")
...
if __name__ == "__main__" or "dbutils" in globals():
    try:
        ...
        try:
            ...
            # If widgets return empty strings, use defaults
            if not catalog_name:
                catalog_name = "demos"
            if not schema_name:
                schema_name = "career_path"
        except:
            # Widgets not set, use defaults
            catalog_name = "demos"
            schema_name = "career_path"
    except NameError:
        # dbutils not available, use defaults
        catalog_name = "demos"
        schema_name = "career_path"
